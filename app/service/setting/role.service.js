const { User, Role, Policy } = require('../../models')

const BaseService = require('../base.service')
const Sequelize = require('sequelize')

const log4js = require('../../../config/log4js');
const { reject } = require('lodash');
const log = log4js.getLogger('role.service.js');

/**
 * 
 * @param {*} page 
 * @param {*} itemPerPage 
 * @param {*} query 
 * @param {*} status 
 */
exports.getAll = (page, itemPerPage, query, status) => new Promise(async (resolve, reject) => {
  let statusData = status ? (status == 'active' ? true : false) : null;

  var queryFilter = query ? { name: { [Sequelize.Op.like]: `%${query}%` } } : null;
  var statusFilter = status ? { is_active: statusData } : null;
  var condition = { [Sequelize.Op.and]: [queryFilter, statusFilter] };

  let attributes = ['id', 'name','description', 'is_active', 'created_at'];

  let responce = await BaseService.paginate(Role, page, itemPerPage, log, condition, attributes);
  resolve(responce)


});

/**
 * 
 * @param {*} values 
 */
exports.create = (values) => new Promise(async (resolve, reject) => {
  Role.create(values).then(async doc => {
    await doc.setPolicies(values.policies)
    resolve(doc)
  })
    .catch((err) => { log.error(err); reject(err) });
});

/**
 * 
 * @param {*} model 
 */
exports.show = (model) => new Promise(async (resolve, reject) => {
  let attributes = ['id', 'name', 'description', 'is_active', 'created_at'];
  let include = [
    {
      model: Policy,
      as: 'policies',
      attributes: ['id', 'name', 'group', 'description', 'model', 'action'],
      through: {
        attributes: [],
      }
    }
  ]

  let responce = await BaseService.show(Role, model, log, attributes, include);
  resolve(responce)
});

/**
 *
 * @param {int} id
 * @param {Object} values
 * @returns{Object}
 */
exports.update = (id, values, tempPassword) => new Promise(async (resolve, reject) => {
  if (!id) reject(new Error(`id can't be empty`));
  Role.findOne({ where: { id: id } })
    .then(async doc => {
      await doc.update(values)
      await doc.setPolicies(values.policies)
      resolve(`User Successfully Updated.`);
    })
    .catch((err) => { log.error(err); reject(err) });
});

/**
 *
 * @param {int} id
 * @returns {String}
 */
exports.delete = (id) => new Promise(async (resolve, reject) => {
  if (!id) reject(new Error(`id can't be empty`));
  let responce = await BaseService.delete(Role, id, log);
  resolve(responce)
});



/**
 * 
 * @param {*} query 
 * @param {*} except 
 * @param {*} status 
 */
exports.search = (query, except, status) => new Promise(async (resolve, reject) => {
  let statusData = status ? (status == 'active' ? true : null) : null;

  let queryFilter = query ? { name: { [Sequelize.Op.like]: `%${query}%` } } : null;
  let selected = except ? { id: { [Sequelize.Op.not]: JSON.parse(except) } } : null;
  let statusFilter = status ? { is_active: statusData } : null;
  let condition = { [Sequelize.Op.and]: [queryFilter, statusFilter, selected] };

  let attributes = ['id', 'name'];
  let values = await BaseService.search(Role, log, condition, attributes);
  resolve(values)
});

/**
 * 
 * @param {*} field 
 * @param {*} value 
 */
exports.count = (field, value) => new Promise(async (resolve, reject) => {
  let values = await BaseService.count(Role, field, value, log);
  resolve(values)
})


/**
*
* @param field
* @param value
* @returns {Promise<unknown>}
*/
exports.find = (field, value) => new Promise(async (resolve, reject) => {
  let values = await BaseService.find(Role, field, value, log);
  resolve(values)
})

exports.softDelete = (id) =>new Promise(async (resolve, reject) => {
    if (!id) reject(new Error(`id can't be empty`));
    let responce = await BaseService.softDelete(Role, id, log);
    resolve(responce)
});